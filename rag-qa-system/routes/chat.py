        ...
        def generate():
            ...
            def vllm_process_task(process_info, question, session_id, selected_model):
                ...
                for user in user_patterns:
                    if user in question:
                        detected_user = user
                        print(f"👤 [vLLM {process_id}] 고객명 감지: {user} (청킹: {chunking_type})")
                        break
                ...
                if detected_user:
                    ...
                    # s3-chunking에서만 개인화 카드 분석 처리
                    if any(keyword in question for keyword in card_keywords) and chunking_type == "custom":
                        print(f"💳 [vLLM {process_id}] 카드 관련 질의 감지 - s3-chunking 개인화 처리")
                        # 개인화 플래그 설정 (s3-chunking에서만)
                        is_personalized = True
                        print(f"✅ [vLLM {process_id}] 개인화 모드 활성화: {detected_user} 고객 (s3-chunking)")
                    elif any(keyword in question for keyword in card_keywords) and chunking_type == "basic":
                        print(f"📝 [vLLM {process_id}] 카드 관련 질의 감지 - s3 기본 모드 (txt파일만 읽기, 이미지 없음)")
                        # s3 기본에서는 txt 파일만 읽어서 텍스트 정보만 표시
                        is_personalized = True  # txt 파일 읽기 위해 개인화 활성화 (이미지 없이)
                    
                    # MCP 카드 분석 시스템 (개인화 활성화 시 실행)
                    if is_personalized:
                        try:
                            print(f"🔍 [vLLM {process_id}] MCP 카드 분석 시스템 시작")
                            
                            # DynamicCardAnalysisService로 통일 (청킹타입별 조건부 처리)
                            from services.card_analysis_service_new import DynamicCardAnalysisService
                            # 기존 벡터DB 매니저 전달 (rag_chain에서 가져오기)
                            existing_vectorstore = getattr(rag_chain, 'dual_vectorstore_manager', None)
                            dynamic_service = DynamicCardAnalysisService(existing_vectorstore)
                            
                            # 청킹 타입별 카드 분석 및 이미지 처리
                            owned_card_images = []
                            ...
                            if chunking_type == "custom":
                                ...
                                card_summary = dynamic_service.format_analysis_response(dynamic_analysis)
                                
                                print(f"  🎯 동적분석: 보유 {len(dynamic_analysis.owned_cards)}, 가능 {len(dynamic_analysis.available_cards)}, 추천 {len(dynamic_analysis.recommended_cards)}")
                                
                                # 보유 카드 이미지 (실제 분석 결과 사용)
                                for card in dynamic_analysis.owned_cards:
                                    if card.image_path:
                                        owned_card_images.append(f'![{card.name}](/images/{card.image_path})')
                                        print(f"  📷 보유카드 이미지: {card.name} → {card.image_path}")
                                
                                # 발급 가능/추천 카드 이미지 (실제 분석 결과 사용)
                                for card in dynamic_analysis.available_cards + dynamic_analysis.recommended_cards:
                                    if card.image_path:
                                        ...
                                        print(f"  📷 발급가능카드 이미지: {card.name} → {card.image_path}")
                                
                                print(f"  🖼️ 최종 이미지: 보유카드 {len(owned_card_images)}개, 발급가능카드 {len(available_card_images)}개")
                                
                                # 카드 발급 절차 이미지 (MD 파일에서는 013.gif가 카드 발급 절차 이미지)
                                process_image = '![카드발급 절차](/images/Aspose.Words.4c2a2064-0c7c-48d5-aca6-c4d7a6eade2b.013.gif)'
                                
                            else:  # s3 (basic) - 텍스트만, 이미지 없음
                                print(f"📝 [vLLM {process_id}] s3기본: 텍스트 전용 모드 (이미지 없이 txt파일만)")
                                
                                # s3 기본에서도 txt 파일은 읽어서 텍스트 정보 표시
                                dynamic_analysis = dynamic_service.analyze_customer_cards(detected_user, "basic")
                                card_summary = dynamic_service.format_analysis_response(dynamic_analysis)
                                
                                print(f"  📄 텍스트분석: 보유 {len(dynamic_analysis.owned_cards)}, 가능 {len(dynamic_analysis.available_cards)}, 추천 {len(dynamic_analysis.recommended_cards)}")
                                print(f"  📝 s3기본: 이미지 제외, 텍스트만 표시")
                            
                            # enhanced_question 생성 전 디버깅 로그
                            print(f"🔍 [vLLM {process_id}] enhanced_question 생성:")
                            print(f"  - card_summary 길이: {len(card_summary)}자")
                            print(f"  - owned_card_images: {len(owned_card_images)}개")
                            for i, img in enumerate(owned_card_images):
                                print(f"    {i+1}. {img}")
                            print(f"  - available_card_images: {len(available_card_images)}개")
                            for i, img in enumerate(available_card_images[:3]):  # 상위 3개만
                                print(f"    {i+1}. {img}")
                            print(f"  - process_image: {len(process_image)}자")
                            
                            # 청킹 타입별 enhanced_question 구성
                            if chunking_type == "basic":
                                # s3 기본: 텍스트만, 이미지 섹션 완전 제거
                                enhanced_question = f"""
질문: {question}
...
위 분석 결과를 바탕으로 {detected_user} 고객에게 BC카드 발급 안내를 제공해주세요.
현재 보유하신 카드 정보를 포함하여 텍스트로만 안내해주세요.
"""
                            else:
                                # s3-chunking: 카드명과 이미지 통합 표시  
                                enhanced_question = f"""
질문: {question}

=== {detected_user} 고객 개인화 카드 현황 ===
💡 **중요**: 아래 분석 결과를 기반으로 답변하세요. 절대 "별도 추천 카드 없음"이라고 하지 마세요!

📊 **동적 분석 완료**: 
- 보유 카드: {len(dynamic_analysis.owned_cards)}장 (우리카드, 하나카드, NH농협카드)
- 발급 가능: {len(dynamic_analysis.available_cards)}장 (미보유 카드들)

{card_summary}

🖼️ **보유 카드 이미지** (절대 번호 변경 금지):
{chr(10).join(owned_card_images) if owned_card_images else '보유한 카드가 없습니다.'}

🆕 **발급 가능 카드 이미지** (절대 번호 변경 금지):
{chr(10).join(available_card_images)}

📋 **카드 발급 절차 이미지**:
{process_image}

위 분석 결과와 이미지를 바탕으로 {detected_user} 고객에게 맞춤형 카드 발급 안내를 제공해주세요.
각 카드명 바로 아래에 해당 이미지를 표시해주세요.
"""
                ...
                try:
                    ...
                    if chunking_type == "basic":
                        ...
                        # 2차: 카드 발급 상세 정보를 위한 키워드 검색 (개인화 강화)
                        detailed_keywords = [
                            "BC카드발급안내 카드발급 절차 STEP",
                            "발급대상 신용카드 체크카드 결제능력",
                            "신청방법 구비서류 회원은행 영업점",
                            "회원은행별 카드발급안내 우리카드 하나카드",
                            "NH농협카드 IBK기업은행 KB국민카드",
                            "DGB대구은행 BNK부산은행 BNK경남은행",
                            "citi은행 신한카드 BC바로카드",
                            "카드 이미지 gif 회원은행",
                            f"{detected_user} 보유 카드 미보유 추천",
                            "개인화 고객 맞춤"
                        ]
                    ...
                    if is_personalized and detected_user:
                        ...
                        if chunking_type == "basic":
                            # s3 기본: 텍스트 전용 프롬프트
                            print(f"📝 [vLLM {process_id}] {detected_user} 고객 s3 기본 텍스트 전용 프롬프트 사용")
                            prompt = f"""당신은 BC카드 전문 상담사입니다. {detected_user} 고객에게 맞춤형 카드 발급 안내를 텍스트로만 제공하세요.
                            
=== {detected_user} 고객 개인화 카드 현황 (텍스트 전용) ===
💡 **중요**: 아래 분석 결과를 기반으로 답변하세요. 절대 "별도 카드 없음"이라고 하지 마세요!

📊 **동적 분석 완료** (이미지 제외): 
- 보유 카드: {len(dynamic_analysis.owned_cards)}장
- 발급 가능: {len(dynamic_analysis.available_cards)}장 (모든 미보유 카드)

{card_summary}

**답변 작성 가이드 (s3 기본 - 텍스트 전용):**
1. **제목 표시**: "**1. 현재 보유 중인 카드**", "**2. 발급 가능 카드**", "**3. BC카드 발급 안내**" 형식으로 굵게 표시
2. **절대 금지**: "별도 발급 가능 카드 없음" 같은 표현 사용 금지
3. **필수 표시**: 위 분석 결과에 있는 모든 카드를 반드시 표시할 것
4. 보유 카드는 "✅ 현재 보유중" 으로 표시
5. 발급 가능 카드는 "🆕 발급가능" 으로 표시  
6. **BC카드 발급 안내 포함**: 제공된 자료에서 카드발급 절차, 발급대상, 신청방법 등을 반드시 포함
7. **이미지 사용 금지**: 이미지 마크다운은 절대 사용하지 마세요
8. **텍스트만**: 카드명과 설명만 텍스트로 표시
9. **구체적 정보 제공**: 발급 절차, 대상, 구비서류 등 실용적 정보 포함

답변:"""
                        else:
                            # s3-chunking: 이미지 포함 프롬프트
                            print(f"🎆 [vLLM {process_id}] {detected_user} 고객 s3-chunking 이미지 포함 프롬프트 사용")
...
**답변 작성 가이드:**
1. **제목 표시 절대규칙**: "**1. 현재 보유 중인 카드**", "**2. 발급 가능 카드**" 형식으로 굵게 표시 (발급 추천 카드 섹션 제거)
2. **절대 금지**: "별도 발급 가능 카드 없음" 같은 표현 사용 금지
3. **필수 표시**: 위 분석 결과에 있는 모든 카드를 반드시 표시할 것
4. 보유 카드는 "✅ 현재 보유중" 으로 표시
5. 발급 가능 카드는 "🆕 발급가능" 으로 표시 (모든 미보유 카드 포함)
6. **🖼️ 카드명과 이미지 통합 표시 필수**: 
   - **형식**: "✅ 우리카드" 바로 다음 줄에 해당 이미지 마크다운 표시
   - **예시**: ✅ 우리카드\n![우리카드](/images/Aspose.Words...014.gif)
   - **금지**: 별도 "이미지" 섹션 생성 금지
   - **절대 규칙**: 위의 "보유 카드 이미지:" 섹션에 제공된 이미지 경로를 정확히 그대로 사용할 것
7. **BC카드 발급 안내 전체 포함**: 자료에 있으면 카드발급 절차, 발급대상, 신청방법 및 구비서류, 회원은행별 카드발급안내를 모두 포함
8. **개인화 정보 우선**: {detected_user} 고객의 보유/미보유 카드 정보를 맨 앞에 배치
9. **실제 데이터만 사용**: 제공된 자료에 있는 내용만 사용하고, 없는 내용은 절대 임의로 생성하지 말 것
10. **마크다운 렌더링 최적화**: 
   - 텍스트와 테이블 사이 공백 최소화 (한 줄 공백만 사용)
   - 각 섹션 간 구분을 위해 적절한 줄바꿈 사용
   - 불필요한 빈 줄 제거하여 컴팩트하게 구성
11. **헤더 사용 금지**: ### 마크다운 헤더는 절대 사용하지 말고 **굵은 글씨**로만 제목 표시
12. **구조화된 출력**: 번호가 있는 제목(**1. 제목**), 카드별 정보(• 이모지), 테이블 형태로 체계적 구성
13. **허위정보 금지**: VIP 혜택, 특별 우대 등 제공된 자료에 없는 내용은 절대 언급하지 말 것

**⚠️ 이미지 경로 절대 규칙**: 
- **중요**: "보유 카드 이미지:" 섹션과 "발급 가능 카드 이미지:" 섹션에 제공된 이미지 경로를 정확히 그대로 복사해서 사용
- ✅ 올바른 예: 우리카드는 반드시 ![우리카드](/images/Aspose.Words.4c2a2064-0c7c-48d5-aca6-c4d7a6eade2b.014.gif)
- ✅ 올바른 예: 하나카드는 반드시 ![하나카드](/images/Aspose.Words.4c2a2064-0c7c-48d5-aca6-c4d7a6eade2b.016.gif)
- ✅ 올바른 예: NH농협카드는 반드시 ![NH농협카드](/images/Aspose.Words.4c2a2064-0c7c-48d5-aca6-c4d7a6eade2b.017.gif)
- ❌ 잘못된 예: 임의로 번호 변경 금지 (013.gif 등)
- **절대 금지**: 제공된 이미지 경로의 번호를 임의로 변경하지 말 것
